...
def match_glim(AirNow_Radar_df = "",GLiM_xr = "",outname=""):
    
        Sitelat = list(AirNow_Radar_df.loc[:,'Latitude'])
        Sitelon = list(AirNow_Radar_df.loc[:,'Longitude'])

        print("retrieving glim values from  geotiff file ......")
        AirNow_Radar_df['glim'] = find_value(Sitelat,Sitelon,GLiM_xr)
...
if __name__ == "__main__":
    ...
    match_glim(AirNow_Radar_df = NEXZ01_filter_NEGPLS,GLiM_xr=da)
    print("-------%s seconds -------" %(time.time() - start_time))