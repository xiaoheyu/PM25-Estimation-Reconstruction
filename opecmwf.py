...
def find_value(Sitelat, Sitelon,xarray,variable):
    ...
    for i in range(len(Sitelat)):
        ...
        # Check i
        try:
            value = xarray.sel(latitude = lat,longitude = lon, method='nearest',tolerance=0.1)[variable].values
            if value == value:
                n += 1
            else: 
...
def match_ecmwf(df = "",ECMWF_xr = "", variables = ['u10','v10','d2m','t2m' ,'lai_hv','lai_lv','sp','sro','tp']):
    ...
    for var in variables:
        ...
        df[var] = find_value(latlist,lonlist,ECMWF_xr,var)
...
def plot_2decmwf (ds,shape_feature):
    ...
    (ds.t2m-275).plot(cmap=plt.cm.coolwarm)
